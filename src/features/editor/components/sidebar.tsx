...
import { LayoutTemplate, Pencil, Presentation, Settings, Shapes, Sparkles, Type, Image } from 'lucide-react';
...
export const Sidebar = ({ activeTool, onChangeActiveTool }: SidebarProps) => {
  return (
    <aside className='bg-white flex flex-col h-full w-[100px] border-r overflow-y-auto'>
      <ul className='flex flex-col '>
        <SidebarItem icon={LayoutTemplate} label='Design' isActive={activeTool === 'templates'} onClick={() => onChangeActiveTool('templates')} />
        <SidebarItem icon={Image} label='Image' isActive={activeTool === 'images'} onClick={() => onChangeActiveTool('images')} />
        ...
        <SidebarItem icon={Sparkles} label='Sparkles' isActive={activeTool === 'ai'} onClick={() => onChangeActiveTool('ai')} />
